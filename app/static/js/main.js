...
async function loadSiteConfig() {
    try {
        ...
        if (logo) {
            logo.src = config.image_logo_url;
        }
        
        // Update banner image
        const banner = document.querySelector('.banner-image');
        if (banner) {
            banner.src = config.image_banner_url;
        }
        ...
        aboutPhotos.forEach((photo, index) => {
            ...
            photo.src = config[`about_photo${photoNumber}_url`];
            photo.alt = config[`about_photo${photoNumber}_alt`];
        });
